...
async function main() {
  ...
  const RadioApp = await hre.ethers.getContractFactory("RadioApp");
  //deploy the money router account using the proper host address and the address of the first signer
  const radioApp = await RadioApp.deploy(sf.settings.config.hostAddress, signers[0].address);

  await radioApp.deployed();

  console.log("RadioApp deployed to:", radioApp.address);
}
