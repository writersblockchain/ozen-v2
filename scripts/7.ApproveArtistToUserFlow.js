...
async function main() {
  ...
  const artistSuperAppContract = new ethers.Contract(
    artistAppAddress,
    UserAppABI,
    provider
  );
  ...
  await artistSuperAppContract
    .connect(signers[0])
    .approveFlow(userAppAddress, ozenSuperTokenAddress, hre.ethers.utils.parseEther("38580000000"))
    .then(function (tx) {
      console.log(`
        Congrats! You just successfully approved a flow! 
        Tx Hash: ${tx.hash}
    `);
    });
}
